...
function AuthorPost() {
    ...
    const loadData = async () => {
        try {
            const res = await axios.get(`http://localhost:8000/getpostbyauthor/${authorId}`);
            if (res.data === "no") {
                msetalert("* Posts not found..");
            } else {
                if (authorname!=="My" && res.data.length > 0) {
    ...
    return (
        <>
            <h4 class="text-muted mt-3 text-center">{authorname} Blogs</h4><hr />
                                ...
                                return (
                                    <>
                                        <div className="col-lg-4 col-sm-6 py-3">
                                            <div className={`card mx-auto ${data.disable ? 'blockedcard' : null}`} title="open blog" style={{ maxWidth: "18rem" }}>
